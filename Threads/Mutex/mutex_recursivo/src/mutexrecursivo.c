...
int main(void){
	...
	/* Transforma mutex_saldo em um mutex recursivo:
	 */
	...
	/*
	* Ao comentar as linhas anteriores o programa irá travar pois o mutex por padrao não é recursivo.
	* As duas funcoes dão lock no mutex(deposita_juros,calcula_juros) de forma sequencial, o que ocorre é
	* que calcula_juros não consegue dar lock no mutex pois já está com lock em
	* deposita_juros, isso é um DEADLOCK, então calcula_juros fica eternamente
	* aguardando.
	*/
